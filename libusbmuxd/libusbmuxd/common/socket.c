...
const wchar_t *socket_addr_to_string(struct sockaddr *addr, wchar_t *addr_out, size_t addr_out_size)
{
#ifdef WIN32
    WSADATA wsa_data;
    if (!wsa_init) {
...
int socket_connect_addr(struct sockaddr *addr, uint16_t port)
{
    ...
    if (sfd < 0) {
        if (verbose >= 2) {
            ...
            fprintf(stderr, "%s: Could not connect to %ls port %d\n", __func__, addrtxt, port);
        }
        return -1;
    }
